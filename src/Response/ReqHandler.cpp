...
{
	//_childOut = std::string("/tmp/.") + std::to_string((long long)this);
...
Response *ReqHandler::HandleDirResource()
{
	...
	if ((index.empty() && !_autoIndex) || (error == 2)) // index not found and no directory listing
		...
		return ResGen.getErrorResponse(_reqCMservers, 404, _hostName, _connection);
	std::string &resource = const_cast<std::string &>(_req.getResource()); // in case of _req.getResource() = /?author=1
//	resource += index; // this will be /?author which is wronge i think resource should be /index.php?author=1 (this comment was working)

	// start of new added part
	q_pos = resource.find_first_of('?');
	if (q_pos != std::string::npos)
		resource.insert(q_pos, index);
	else 
		resource += index;
...
Response *ReqHandler::HundleCGI()
{
	...
	_childOut = std::string("/tmp/.") + std::to_string(1000000 * tv.tv_sec + tv.tv_usec);
	int PID;
	...
	int outFD = open(_childOut.c_str(), O_RDWR | O_CREAT, 0777);
	std::cout << "\033[32mcreated: " << _childOut << "\033[0m\n";
	if (outFD == -1)
			return ResGen.getErrorResponse(_reqCMservers, 500, _hostName, _connection);
...
void ReqHandler::addRequestHeaders(std::vector<std::string> &envHeaders)
{
	...
	for (size_t i = 0; i < size; i++)
	{
		if (_req.aHeaders[i].first == "Content-Type")
			...
			continue;
		else if (_req.aHeaders[i].first == "Host")
			continue ;
		else
